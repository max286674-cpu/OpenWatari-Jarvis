...
# Russian + responsive voice defaults.
...
# Real OpenRouter routing. Previously openrouter:qwen silently fell through to the freellmapi client.
...
# Use a fast, cheap, non-thinking Qwen model as the normal voice brain if an OpenRouter key exists.
env=ROOT/'.env'
if env.exists():
    ...
    updates={
        ...
        'JARVIS_TTS_AFFECT_ENABLED':'false',
    ...
    env.write_text('\n'.join(out)+'\n',encoding='utf-8')
    print('UPDATED .env (key preserved, not printed)')
else:
    print('WARNING: .env not found; OpenRouter key must be configured locally')

# Remove synthetic speech that caused "Занимаюсь / Одну секунду / Переключаюсь" spam.
...
old='''        task = asyncio.ensure_future(coro)\n        interval = max(0.05, settings.tool_slow_warn_seconds)'''
if old not in s: raise SystemExit('tool watchdog anchor missing')
new='''        task = asyncio.ensure_future(coro)\n        # Never let a tool freeze a voice turn indefinitely.\n        timeout = max(8.0, min(30.0, float(settings.tool_slow_warn_seconds) * 3.0))\n        try:\n            return await asyncio.wait_for(task, timeout=timeout)\n        except asyncio.TimeoutError:\n            task.cancel()\n            try: await task\n            except asyncio.CancelledError: pass\n            raise TimeoutError(f"tool {name} exceeded {timeout:.0f}s timeout")'''
# replace the entire watchdog method, not just its prefix
...
# Deterministic common Windows commands: no LLM can hallucinate "I opened it" anymore.
...
helper=r'''    async def _direct_pc_command(self, text: str):
        ...
        apps={
            ...
            'проводник':'explorer.exe','хром':'chrome.exe','chrome':'chrome.exe',
            'гугл хром':'chrome.exe','firefox':'firefox.exe','фаерфокс':'firefox.exe',
            'дискорд':'discord.exe','телеграм':'telegram.exe','telegram':'telegram.exe',
            'спотифай':'spotify.exe','spotify':'spotify.exe','стим':'steam.exe','steam':'steam.exe',
            'код':'code.exe','vs code':'code.exe','вс код':'code.exe'}
        ...
        if m:
            ...
            if target.startswith(('http://','https://','www.')) or '.com' in target or '.ru' in target:
                return await open_url({'url':target})
        if re.match(r'^(?:джарвис[,.]?\s*)?(?:пауза|поставь.*паузу)(?:\s+видео|\s+музыку)?$',t):
            return await media_pause({})
        if re.match(r'^(?:джарвис[,.]?\s*)?(?:продолжи|возобнови)(?:\s+видео|\s+музыку)?$',t):
            return await media_pause({})
        return None
...
# Reliable Windows app launch: ShellExecute first, PowerShell Start-Process fallback.
