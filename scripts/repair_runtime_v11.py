...
The repair restores known-good canonical files, applies small exact patches, and validates syntax. It is
safe to run repeatedly and never performs structural regex slicing.
"""
...
def patch_wake():
    ...
    # Add custom local Russian model ahead of pretrained English models when the file exists.
    anchor='''    models: list[str] = []\n    pending: list[str] = []\n    for phrase in desired:'''
    replacement='''    models: list[str] = []\n    pending: list[str] = []\n    try:\n        from jarvis.config import settings\n        custom = Path(settings.wake_word_custom_model_path).expanduser()\n        if custom.exists() and custom.suffix.lower() in {".tflite", ".onnx"}:\n            # A trained Russian model is authoritative; don't mix it with the English pretrained model.\n            return [str(custom.resolve())], []\n    except Exception:\n        pass\n    for phrase in desired:'''
    if anchor in s and 'custom = Path(settings.wake_word_custom_model_path)' not in s:
        s=s.replace('import asyncio\n','import asyncio\nfrom pathlib import Path\n',1).replace(anchor,replacement,1)
    old='''        self._model = Model(wakeword_models=models, inference_framework="onnx")'''
    new='''        framework = "tflite" if any(str(m).lower().endswith(".tflite") for m in models) else "onnx"\n        self._model = Model(wakeword_models=models, inference_framework=framework)'''
    s=s.replace(old,new)
    ast.parse(s,filename=rel);return write_if_changed(rel,s) if s!=original else False

def validate():
    checks={"src/jarvis/brain/agent.py":("_PURE_CHAT_RE = re.compile(","def _is_pure_chat(","def _is_affirmation(","def _execute_calls("),"src/jarvis/brain/intent_router.py":("def forced_tools(",),"src/jarvis/brain/tools/desktop.py":("desktop_action","desktop_screenshot"),"src/jarvis/edge/wake_word.py":("wake_word_custom_model_path","inference_framework=framework")}
...
def main():
    changed=restore_canonical_files();changed=patch_agent() or changed;changed=patch_config() or changed;changed=patch_tools_registry() or changed;changed=patch_router() or changed;changed=patch_wake() or changed;validate();print("runtime repair: OK (canonical restore + idempotent patches)"+("; files updated" if changed else "; already clean"))
