...
class OutputKind(str, Enum):
    ...
    phone_speaker = "phone_speaker"      # phone loudspeaker — SHARED
    speakers = "speakers"                # laptop/desktop open speakers — SHARED
    unknown = "unknown"
...
# ---- Phase 6: the four supported devices, made explicit ---------------------------------
# Each is "enabled": Jarvis serves it from the same brain, classifies its endpoint, and routes
# barge-in/audio accordingly. A phone/glasses client connects over the edge<->brain protocol and
# declares its `device_id`; the laptop is the local host running the full pipeline.
@dataclass(frozen=True)
class DeviceRoute:
    device_id: str
    label: str
    kind: OutputKind
    is_local: bool          # True only for this host laptop
    barge_in: bool          # derived: private endpoint -> barge-in on


SUPPORTED_DEVICES: dict[str, dict] = {
    "laptop": {"label": "This host laptop", "kind": OutputKind.speakers, "is_local": True},
    "iphone": {"label": "iPhone", "kind": OutputKind.phone_speaker, "is_local": False},
    "airpods": {"label": "AirPods Pro Max", "kind": OutputKind.headphones, "is_local": False},
...
_DEVICE_ALIASES = {
    ...
    "iphone": "iphone", "phone": "iphone", "ios": "iphone",
...
def resolve_device_route(device: str | None, headphones_connected: bool = False) -> DeviceRoute:
    ...
    ``headphones_connected`` implements the user's rule: if AirPods Pro Max are connected to
    EITHER the phone or the laptop, route everything to the headphones (private) and turn barge-in
    on — regardless of the base device. Unknown ids fall back to the laptop/host.
    """
