...
async def ttft(client, model):
    ...
    try:
        s = await client.chat.completions.create(
            model=model, messages=[{"role":"user","content":PROMPT}], stream=True, temperature=0.4)
        async for ch in s:
...
async def main():
    c = AsyncOpenAI(base_url=settings.freellmapi_base_url, api_key=settings.freellmapi_api_key or "x", timeout=30, max_retries=0)
